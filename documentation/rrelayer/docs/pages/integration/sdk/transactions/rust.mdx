...
use anyhow::Result;
use rrelayer::{
    AdminClient, CreateClientAuth, CreateClientConfig, EvmAddress, RelayTransactionRequest,
    SendTransactionResult, TransactionData, TransactionSpeed, TransactionValue,
    create_client, ChainId,
};
use std::str::FromStr;
use dotenvy::dotenv;
use std::env;

async fn get_admin_client() -> Result<AdminClient> {
    dotenv().ok();
...
async fn example() -> Result<()> {
    let admin_client = get_admin_client().await?;
    
    let chain_id = ChainId::from(1); // Ethereum mainnet
    ...
    let request = RelayTransactionRequest {
        to: EvmAddress::from_str("0x5FCD072a0BD58B6fa413031582E450FE724dba6D")?, // ERC20 contract address
        value: TransactionValue::zero(),
        data: TransactionData::from(encoded_data),
        speed: Some(TransactionSpeed::FAST),
        external_id: None,
    ...
    let result: SendTransactionResult = admin_client.network()
        .send_random(&chain_id, &request, None).await?;
    println!("Contract transaction sent via random relayer: {:?}", result);

    let relay_transaction_result = admin_client.transaction()
        .wait_for_transaction_receipt_by_id(&result.id).await?;
