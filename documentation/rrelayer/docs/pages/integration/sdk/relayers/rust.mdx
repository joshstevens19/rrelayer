...
:::info
Only basic auth authentication can import relayers. This endpoint is only available when using signing providers that support key import.
:::
...
```rust
pub type Response = ApiResult<ImportRelayerResult>; // [!code focus]
```
...
use anyhow::Result;
use rrelayer::{
    Client, CreateClientAuth, CreateClientConfig, CreateRelayerResult, EvmAddress, ImportRelayerResult,
...
async fn example() -> Result<()> {
    ...
    // Import an existing KMS key as a relayer
    let result: ImportRelayerResult = client
        .relayer()
        .import(
            11155111, // chainId
            "my-imported-relayer", // name
            "arn:aws:kms:us-east-1:123456789012:key/abc123-def456-ghi789", // keyId (KMS key ARN)
            &EvmAddress::from_str("0x742d35cc6466c4b0de3e3e8c7b8e8f9e8a8d8c8b")?, // address (must match the key's actual address)
        )
        .await?;

    println!("Imported relayer: {:?}", result);
    // result.id = relayer id
    // result.address = relayer address
    // result.wallet_index = assigned wallet index
    // result.key_alias = KMS alias created (e.g., "alias/rrelayer-wallet-0-11155111")

    let relayer_client = client
        .get_relayer_client(&result.id, Some(TransactionSpeed::FAST))
        .await?;
...
:::warning
The provided `address` must match the actual Ethereum address derived from the key. The endpoint verifies this before creating any side effects. If the addresses don't match, the request will fail with a 400 error.
:::
