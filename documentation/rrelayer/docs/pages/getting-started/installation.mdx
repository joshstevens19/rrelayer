...
:::info
rrelayer uses docker to spin up postgres databases for you when it runs locally so It's recommended to install [docker](https://www.docker.com/products/docker-desktop/)
...
```bash
docker run -it -v $PWD:/app/project ghcr.io/joshstevens19/rrelayer new -p /app/project
```
...
docker run -it -v $PROJECT_PATH:/app/project ghcr.io/joshstevens19/rrelayer start  -p /app/project
```
