...
rrelayerr is an opensource relayer build in rust which can be used to send gasless transactions, transaction migrations, L2 proof transactions, blob storage transactions, or general script usage.
rrelayerr does not have a single usecase. rrelayerr gives you a full out the box solution. It will bump up transactions if gas changes and will get the transaction over the line for you.
...
rrelayerr ships with a fully fledger dashboard which you can read more about in the [dashboard section](/docs/dashboard/introduction).
The dashboard allows you to create new relayers, add new API keys, config the relayers for their needs and more.

## SDKs

rrelayerr ships with a fully fledger SDKs to allow you to integrate it easily into your application. With full support in viem and ethers if you are using `typescript` in your application.
