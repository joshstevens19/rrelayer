...
Lets install both `viem` and `rrelayer-ts`
...
The rrelayer client exposes ways for you to create an relayer client which exposes a ethereum provider you can use with `viem`.
...
// viem wallet client to now allow signing as we injected rrelayer provider
